        ...
        def show_audio(state):
            ...
            for idx, f in enumerate(state):
                ...
                with gr.Accordion(label=label, open=True):
                    ...
                    categorys = src.config_manager.ConfigManager().get_category_list()

                    for e in categorys:
                        logging.info("Ca: " + e)

                    category = gr.Dropdown(label="Choose a Category", choices=categorys, interactive=True)
                    audio_type = gr.Dropdown(label="Choose a Audio Type", value="Meeting", choices=["Meeting", "Media", "Generic"], interactive=True)
                    summary = gr.Textbox(
